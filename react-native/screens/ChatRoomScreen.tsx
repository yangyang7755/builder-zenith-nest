...
export default function ChatRoomScreen() {
  ...
  const title = route.params?.clubId ? "Club chat" : route.params?.userId ? "Direct message" : "Chat";

  const send = () => {
    if (!text.trim()) return;
    setMessages((prev) => [...prev, { id: Date.now().toString(), sender: "You", text, time: "now" }]);
    setText("");
  };
...
const styles = StyleSheet.create({
  ...
  inputRow: { flexDirection: 'row', alignItems: 'center', padding: 12, borderTopWidth: 1, borderTopColor: '#e5e7eb' },
  input: { flex: 1, borderWidth: 1, borderColor: '#e5e7eb', borderRadius: 999, paddingHorizontal: 14, paddingVertical: 10, marginRight: 8 },
  send: { backgroundColor: '#1F381F', paddingHorizontal: 16, paddingVertical: 10, borderRadius: 999 },
});
