...
const CLUBS = [
  {
    ...
    last: "Training ride tomorrow! Meet at Radcliffe Camera 7am",
    unread: false,
  },
  {
    id: "westway-climbing",
    name: "Westway Climbing Centre",
    last: "New routes set this week! Come check them out 🧗‍♀️",
    unread: true,
  },
];
    ...
    <View style={styles.container}>
      ...
      >
        {FILTERS.map((f) => (
          ...
          >
            <Text
              style={[styles.chipText, active === f && styles.chipTextActive]}
            >
              {f}
            </Text>
          </TouchableOpacity>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 16 }}>
        ...
        {CLUBS.map((c) => (
          <TouchableOpacity
            key={c.id}
            style={styles.item}
            onPress={() => navigation.navigate("ChatRoom", { clubId: c.id })}
          >
        ...
        {DMS.map((m) => (
          <TouchableOpacity
            key={m.id}
            style={styles.item}
            onPress={() => navigation.navigate("ChatRoom", { userId: m.id })}
          >
