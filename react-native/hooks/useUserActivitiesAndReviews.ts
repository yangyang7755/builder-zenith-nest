...
export function useUserActivitiesAndReviews(userId?: string) {
  ...
  const fetchData = async () => {
    ...
    try {
      // Fetch user's activities and reviews in parallel
      ...
      if (activitiesResult.status === 'fulfilled' && activitiesResult.value.data) {
        ...
        console.warn('Failed to fetch activities:', activitiesResult.reason);
      }
      ...
      if (reviewsResult.status === 'fulfilled' && reviewsResult.value.data) {
        ...
        console.warn('Failed to fetch reviews:', reviewsResult.reason);
      }

      // If both API calls failed, use demo data
      if (activities.length === 0 && reviews.length === 0) {
        setData(getDemoActivitiesAndReviews());
        setLoading(false);
        return;
      }
