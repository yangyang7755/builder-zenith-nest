...
export function useUserActivitiesAndReviews(userId?: string) {
    ...
    try {
      ...
      const hasNetworkErrors =
        (activitiesResponse.error &&
          isNetworkError(activitiesResponse.error)) ||
      ...
      if (
        ...
        console.warn(
          "Activities fetch error (non-network):",
          activitiesResponse.error,
        );
      }
      ...
      if (reviewsResponse.error && !isNetworkError(reviewsResponse.error)) {
        console.warn(
          "Reviews fetch error (non-network):",
      ...
      const completedActivities = activities.filter(
        (activity: any) =>
          activity.status === "completed" && activity.organizer_id !== userId,
      );
      ...
      setData({
        completedActivities: enhanceActivitiesWithReviews(
          completedActivities,
          reviews,
        ),
        organizedActivities: enhanceActivitiesWithReviews(
          organizedActivities,
          reviews,
        ),
  ...
  const enhanceActivitiesWithReviews = (
    activities: any[],
    reviews: any[],
  ): ActivityWithReviews[] => {
    return activities.map((activity) => {
      const activityReviews = reviews.filter(
        (review) => review.activity_id === activity.id,
      );
      const averageRating =
