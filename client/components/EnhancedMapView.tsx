...
export default function EnhancedMapView({
  ...
  const mockActivityLocations = {
    ...
    "Regent's Park": { lat: 51.5314, lng: -0.1560 },
  ...
  const activitiesWithCoords = activities.map(activity => {
    ...
    const coordinates = location ? mockActivityLocations[location as keyof typeof mockActivityLocations] : {
  ...
  const useDemoLocation = () => {
    ...
    toast({
      ...
      description: "Using London as demo location. Nearby activities calculated.",
    });
  };
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              {userLocation ? `${nearbyActivities.length} activities within ${radiusKm}km` : 'Getting location...'}
            </p>
          </div>
        </div>
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-xs text-gray-400">
              In production, this would show an interactive map with pins for each activity
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="mb-4">
          ...
          {userLocation && (
            <p className="text-sm text-gray-600">
              Found {nearbyActivities.length} activities within {radiusKm}km of your location
            </p>
          )}
        </div>

        <div className="space-y-3">
          {(userLocation ? nearbyActivities : activitiesWithCoords).map((activity) => (
            <Card 
              key={activity.id} 
        ...
        {nearbyActivities.length === 0 && userLocation && (
          <Card>
            <CardContent className="text-center py-8">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">No Nearby Activities</h3>
