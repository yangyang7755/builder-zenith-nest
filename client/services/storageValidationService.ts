...
class StorageValidationService {
   ...
   */
  async validateUserDataPersistence(
    userId: string,
    ...
    try {
      ...
      result.errors.push(
        `Storage validation failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
      return result;
    }
  }
   ...
   */
  private async validateReviewsStorage(
    userId: string,
    ...
    try {
      ...
      if (reviewsResponse.error) {
        result.warnings.push(
          `Could not validate reviews: ${reviewsResponse.error}`,
        );
        return;
      }
      ...
      if (reviews.length > 0) {
        console.log(
          `✓ Reviews storage validated: ${reviews.length} reviews found`,
        );
      }
    } catch (error) {
      result.errors.push(
        `Reviews validation error: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
   ...
   */
  private async validateFollowersStorage(
    userId: string,
    result: StorageValidationResult,
  ): Promise<void> {
    try {
      ...
      if (followersResponse.error) {
        result.warnings.push(
          `Could not validate followers: ${followersResponse.error}`,
        );
      } else {
      ...
      if (followingResponse.error) {
        result.warnings.push(
          `Could not validate following: ${followingResponse.error}`,
        );
      } else {
        ...
        for (const follow of following) {
          if (!follow.id || !follow.follower_id || !follow.following_id) {
            result.errors.push(
              "Following relationship missing required fields",
            );
          }
        }
      }
      ...
      if (!statsResponse.error && statsResponse.data) {
        ...
        if (stats.followers !== result.summary.followersCount) {
          result.warnings.push(
            "Follower count mismatch between detailed data and stats",
          );
        }
        if (stats.following !== result.summary.followingCount) {
          result.warnings.push(
            "Following count mismatch between detailed data and stats",
          );
        }
      }

      console.log(
        `✓ Followers storage validated: ${result.summary.followersCount} followers, ${result.summary.followingCount} following`,
      );
    } catch (error) {
      result.errors.push(
        `Followers validation error: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
   ...
   */
  private async validateSavedActivitiesStorage(
    userId: string,
    result: StorageValidationResult,
  ): Promise<void> {
    try {
      ...
      if (savedResponse.error) {
        result.warnings.push(
          `Could not validate saved activities: ${savedResponse.error}`,
        );
        return;
      }
      ...
      console.log(
        `✓ Saved activities storage validated: ${savedActivities.length} saved activities`,
      );
    } catch (error) {
      result.errors.push(
        `Saved activities validation error: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
   ...
   */
  private async validateChatMessagesStorage(
    userId: string,
    result: StorageValidationResult,
  ): Promise<void> {
    try {
      ...
      try {
        ...
        const testResponse = await apiService.getDirectMessages(
          "test-user-id",
          1,
          0,
        );
        ...
        if (
          !testResponse.error ||
          testResponse.error.includes("Authentication") ||
          testResponse.error.includes("Not found")
        ) {
          ...
          result.warnings.push(
            `Chat messages validation inconclusive: ${testResponse.error}`,
          );
        }
      } catch (error) {
      ...
      result.errors.push(
        `Chat messages validation error: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
  ...
  async testReviewPersistence(
    ...
    try {
      ...
      if (testReview) {
        ...
        console.warn(
          "✗ Review persistence test failed: Review not found after creation",
        );
        return false;
      }
    } catch (error) {
  ...
  async testFollowPersistence(targetUserId: string): Promise<boolean> {
    try {
      ...
      const followingResponse =
        await apiService.getUserFollowing("current-user"); // This would need the actual current user ID
      ...
      if (testFollow) {
        ...
        console.warn(
          "✗ Follow persistence test failed: Follow relationship not found after creation",
        );
        return false;
      }
    } catch (error) {
  ...
  async validateDataPersistenceAcrossReloads(): Promise<boolean> {
    try {
      ...
      if (previousTime) {
        ...
        if (timeDiff < 60000) {
          // Within 1 minute, likely a reload test
      ...
      console.log(
        "Reload test setup completed. Refresh the page to test persistence.",
      );
      return true;
    } catch (error) {
