...
const ClubMembershipContext = createContext<
  ClubMembershipContextType | undefined
>(undefined);
...
export function ClubMembershipProvider({ children }: { children: ReactNode }) {
  ...
  const [clubMembers, setClubMembers] = useState<Map<string, ClubMembership[]>>(
    new Map(),
  );
  ...
  const joinClub = async (
    ...
    try {
      ...
      showMembershipNotification(
        `You joined ${clubData.displayName}!`,
        "success",
      );
    ...
    try {
      ...
      setClubMembers((prev) => {
        ...
        const filtered = existing.filter(
          (m) => m.user_id !== currentUserProfile.id,
        );
      ...
      showMembershipNotification(
        `You left ${membership.club_data.displayName}`,
        "info",
      );
  ...
  const isClubMember = (clubId: string): boolean => {
    return userMemberships.some(
      (m) => m.club_id === clubId && m.status === "active",
    );
  };

  const getClubRole = (
    clubId: string,
  ): "member" | "manager" | "admin" | null => {
    ...
    try {
      ...
      if (userId === currentUserProfile?.id) {
        setUserMemberships((prev) =>
          prev.map((m) => (m.club_id === clubId ? { ...m, role: newRole } : m)),
        );
      }
...
export function useClubMembership() {
  ...
  if (context === undefined) {
    throw new Error(
      "useClubMembership must be used within a ClubMembershipProvider",
    );
  }
  return context;
}
