...
const validation = await storageValidationService.validateUserDataPersistence(userId);

// Generate health report
const report = await storageValidationService.generateStorageHealthReport(userId);
```
...
const followPersisted = await storageValidationService.testFollowPersistence(targetUserId);
```
...
const health = await storageValidationService.validateUserDataPersistence(userId);
